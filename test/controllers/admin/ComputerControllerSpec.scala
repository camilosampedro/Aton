...
class ComputerControllerSpec extends PlaySpec with MockitoSugar with BeforeAndAfterAll {
  ...
  when(computerService.add(any[Computer])) thenReturn Future.successful(state.ActionCompleted)
  when(userService.checkAndGet(any[LoginFormData])) thenReturn Future.successful(Some(User("", "", None, Role.Administrator)))
  ...
  "ComputerController" should {
    "add a new computer" in {
      ...
      lazy val controller = new ComputerController(computerService, roomService, messagesApi)
